...
setupdefaultconfig(Switch) {
    ...
    gfcGUIconfig=
        (LTRIM
            Experiment::blank
            %A_Tab%;; 1. Grouping
            %A_Tab%PotsPerGroup:Edit|Type:Integer|Default:7|String:"Set the number of pots per group/combination"|TTIP:[Facet2D==TRUE]\nHere, combination |ctrlOptions:number|Tab3Parent:1. Grouping|Link:"https://www.google.com"|Linktext:?|ConfigSection:Experiment
            %A_Tab%UniqueGroups:Edit|Type:String|Default:G14,G21,G28,G35,G42,UU|String:"Set the comma-separated list of all unique group names"|TTIP:If you set 'Facet2D' to TRUE, this must have as many entries as 'Facet2DVar'|Tab3Parent:1. Grouping|Link:"https://www.google.com"|Linktext:?|ConfigSection:Experiment
            %A_Tab%GroupsOrderX:Edit|Type:String|Default:UU,G14,G21,G28,G35,G42|String:"Set the comma-separated order of groups in the plots along X-axis"|TTIP:Order the Groups along the X-Axis. Groups are ordered left to right|Tab3Parent:1. Grouping|Link:"https://www.google.com"|Linktext:?|ConfigSection:Experiment
            %A_Tab%GroupsOrderY:Edit|Type:String|Default:|String:"Set the comma-separated order of groups in the plots along Y-Axis (only for facetting)"|TTIP:[Facet2D==TRUE]\nOrder the Groups along the Y-Axis. Groups are ordered top to bottom.|Tab3Parent:1. Grouping|Link:"https://www.google.com"|Linktext:?|ConfigSection:Experiment
            %A_Tab%RefGroup:Edit|Type:String|Default:UU|String:"Set the reference group for all statistical tests"|TTIP:\n[Facet2D==FALSE]\nFor a normal plot, this must be a member of 'UniqueGroups'\n\n[Facet2D==TRUE]\nFor a facetted plot, this must be a combination of 1 member of 'Facet2DVar' and 'UniqueGroups', separated by a dot (.).\nThe order is always '[UniqueGroups_Member].[Facet2DVar_Member]\nExample:\nUngestresst.Unbehandelt'|Tab3Parent:1. Grouping|Link:"https://www.google.com"|Linktext:?|ConfigSection:Experiment
            %A_Tab%Facet2D:Checkbox|Type:boolean|Default:0|String:"Do you want to facet the plot, f.e. over a treatment?"|Tab3Parent:1. Grouping|Link:"https://www.google.com"|Linktext:?|ConfigSection:Experiment
            %A_Tab%Facet2DVar:Edit|Type:String|Default:""|String:"Set the comma-separated list of facet-members to assing to the 'UniqueGroups'"|TTIP:[Facet2D==TRUE]\nClarification: The entries you specified for 'UniqueGroups' must each match a single entry in this list as well|Tab3Parent:1. Grouping|Link:"https://www.google.com"|Linktext:?|ConfigSection:Experiment
            %A_Tab%;;
            %A_Tab%Palette_Boxplot:Edit|Type:String|Default:"yellow","orange","orangered","red","darkred","black","white"|String:Set the colors for the Summaryplot|TTIP:Colors are assigned regardless of order of the plots, and always in the following order:\nLeft to right, Top to bottom\n\nThis set of colors is responsible for the overview plot|Tab3Parent:1. Grouping|Link:"https://www.google.com"|ctrlOptions:w400|Linktext:?|ConfigSection:Experiment
            ...
            %A_Tab%Normalise:Checkbox|Type:boolean|Default:1|String:"Do you want to normalise your leaf area?"|TTIP:This accesses the data-column 'plant_area_normalised'. For more info, check the documentation.|Tab3Parent:2. GeneralConfiguration|Link:"https://www.google.com"|Linktext:?|ConfigSection:Experiment
            ...
            %A_Tab%axis_units_x:Edit|Type:String|Default:Tage,days|String:"Set the unit of the X-axis (for the Overview-plot)."|TTIP:Format: '[German Text],[English Text]'. Replace a field with "/" to skip it|Tab3Parent:4. Axes|Link:"https://www.google.com"|Linktext:?|ConfigSection:General
            %A_Tab%axis_units_y:Edit|Type:String|Default:cm^2,cm^2|String:"Set the unit of the Y-axis (for the Overview-plot)."|TTIP:Format: '[German Text],[English Text]'. Replace a field with "/" to skip it|Tab3Parent:4. Axes|Link:"https://www.google.com"|Linktext:?|ConfigSection:General
            %A_Tab%axis_units_x_Daily:Edit|Type:String|Default:/,/|String:"Set the unit of the X-axis (for the daily plots)."|TTIP:Format: '[German Text],[English Text]'. Replace a field with "/" to skip it|Tab3Parent:4. Axes|Link:"https://www.google.com"|Linktext:?|ConfigSection:General
            %A_Tab%axis_units_y_Daily:Edit|Type:String|Default:cm^2,cm^2|String:"Set the unit of the Y-axis (for the daily plots)."|TTIP:Format: '[German Text],[English Text]'. Replace a field with "/" to skip it|Tab3Parent:4. Axes|Link:"https://www.google.com"|Linktext:?|ConfigSection:General
